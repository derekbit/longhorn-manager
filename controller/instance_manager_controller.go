...
func (imc *InstanceManagerController) deleteOrphans(im *longhorn.InstanceManager, isInstanceManagerTerminating bool) error {
	...
	orphanList, err := imc.ds.ListOrphansForEngineAndReplicaInstancesByInstanceManagerRO(im.Name)
	if err != nil {
		return err
	}
	...
	for _, orphan := range orphanList {
		...
		case longhorn.OrphanTypeEngineInstance:
			...
			instanceCRScheduledBack, err = imc.isEngineScheduledInCurrentInstanceManager(instanceManager, instanceName)
		case longhorn.OrphanTypeReplicaInstance:
			_, instanceExist = im.Status.InstanceReplicas[instanceName]
			instanceCRScheduledBack, err = imc.isReplicaScheduledInCurrentInstanceManager(instanceManager, instanceName)
		}
		if err != nil {
...
func (imc *InstanceManagerController) isEngineScheduledInCurrentInstanceManager(instanceManager string, instance string) (bool, error) {
	existEngine, err := imc.ds.GetEngineRO(instance)
	switch {
	case err == nil:
		// Engine CR still exists - check the ownership
		return imc.isInstanceScheduledToCurrentInstanceManager(instanceManager, &existEngine.ObjectMeta, &existEngine.Spec.InstanceSpec, &existEngine.Status.InstanceStatus), nil
	case apierrors.IsNotFound(err):
		// Engine CR not found - instance is orphaned
		return false, nil
	default:
		// Unexpected error - unable to check if engine instance is orphaned or not
		return false, errors.Wrapf(err, "failed to check CR scheduled node for engine orphan %v", instance)
	}
}

func (imc *InstanceManagerController) isReplicaScheduledInCurrentInstanceManager(instanceManager string, instance string) (bool, error) {
	existReplica, err := imc.ds.GetReplicaRO(instance)
	switch {
	case err == nil:
		// Replica CR still exists - check the ownership
		return imc.isInstanceScheduledToCurrentInstanceManager(instanceManager, &existReplica.ObjectMeta, &existReplica.Spec.InstanceSpec, &existReplica.Status.InstanceStatus), nil
	case apierrors.IsNotFound(err):
		// Replica CR not found - instance is orphaned
		return false, nil
	default:
		// Unexpected error - unable to check if replica instance is orphaned or not
		return false, errors.Wrapf(err, "failed to check CR scheduled node for replica orphan %v", instance)
	}
}

// isInstanceScheduledToCurrentInstanceManager returns true only when it is very certain that an instance is scheduled in a given instance manager
func (imc *InstanceManagerController) isInstanceScheduledToCurrentInstanceManager(instanceManager string, meta *metav1.ObjectMeta, spec *longhorn.InstanceSpec, status *longhorn.InstanceStatus) bool {
	if !meta.DeletionTimestamp.IsZero() {
		imc.logger.Debugf("Skipping orphan location check; Instance %s is marked for deletion", meta.Name)
		return false
	}
	...
	if status.CurrentState != spec.DesireState || status.OwnerID != spec.NodeID {
		...
		}).Debugf("Skipping orphan location check; Instance %s is in state transition", meta.Name)
		return false
	}
...
func (m *InstanceManagerMonitor) syncOrphans(im *longhorn.InstanceManager, instanceMap instanceProcessMap) {
	engineProcesses, replicaProcesses := m.categorizeProcesses(instanceMap)
	existOrphansList, err := m.ds.ListOrphansForEngineAndReplicaInstancesByInstanceManagerRO(im.Name)
	if err != nil {
...
func (m *InstanceManagerMonitor) createOrphanForInstances(existOrphans map[string]bool, im *longhorn.InstanceManager, instanceMap instanceProcessMap, orphanType longhorn.OrphanType, orphanFilter func(instanceName, instanceManager string) (bool, error)) {
	for instanceName, instance := range instanceMap {
		...
		case longhorn.InstanceStateStarting:
			// Status transitioning. Will handle this after running.
			continue
		case longhorn.InstanceStateStopping, longhorn.InstanceStateStopped:
			// Terminating. No orphan CR needed, and the orphaned instances will be cleanup by instance manager after stopped.
			continue
		}
