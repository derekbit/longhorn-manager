...
func (imc *InstanceManagerController) syncInstanceStatus(im *longhorn.InstanceManager) error {
	if im.Status.CurrentState == longhorn.InstanceManagerStateStopped ||
		...
		im.Status.InstanceEngineTargets = nil
...
func (imc *InstanceManagerController) areDangerZoneSettingsSyncedToIMPod(im *longhorn.InstanceManager) (isSynced, isPodDeletedOrNotRunning, areInstancesRunningInPod bool, err error) {
	...
	for _, instance := range types.ConsolidateInstances(im.Status.InstanceEngines, im.Status.InstanceEngineTargets, im.Status.InstanceReplicas) {
...
func formatInstanceMessage(im *longhorn.InstanceManager) string {
	msg := ""
	if len(im.Status.InstanceEngines)+len(im.Status.InstanceEngineTargets) > 0 {
		msg = fmt.Sprintf("InstanceEngines and InstanceEngineTargets count %v;", len(im.Status.InstanceEngines)+len(im.Status.InstanceEngineTargets))
		i := 0
		...
		for k := range im.Status.InstanceEngineTargets {
			msg = msg + " " + k
			if i++; i >= ITERATE_NAME_LIMIT {
				break
			}
...
func (imc *InstanceManagerController) canDeleteInstanceManagerPDB(im *longhorn.InstanceManager) (bool, string, error) {
	...
	if im.Spec.Type == longhorn.InstanceManagerTypeEngineTarget {
		if len(im.Status.InstanceEngineTargets) == 0 {
			return true, "", nil
		}
		return false, fmt.Sprintf("some instances are still running %v", formatInstanceMessage(im)), nil
	}
...
func (imc *InstanceManagerController) deleteOrphans(im *longhorn.InstanceManager, isInstanceManagerTerminating bool) error {
	...
	for _, orphan := range orphanList {
		...
		case longhorn.OrphanTypeReplicaTargetInstance:
			// TODO: handle engine target orphan deletion
...
func (m *InstanceManagerMonitor) updateInstanceMap(im *longhorn.InstanceManager, instanceMap instanceProcessMap) bool {
	switch {
	default:
		engineProcesses, engineTargetProcesses, replicaProcesses := m.categorizeProcesses(instanceMap)
		...
		if ((len(im.Status.InstanceEngines) == 0 && len(engineProcesses) == 0) || reflect.DeepEqual(im.Status.InstanceEngines, engineProcesses)) &&
			((len(im.Status.InstanceEngineTargets) == 0 && len(engineProcesses) == 0) || reflect.DeepEqual(im.Status.InstanceEngineTargets, engineProcesses)) &&
		...
		im.Status.InstanceEngineTargets = engineTargetProcesses
...
func (m *InstanceManagerMonitor) syncOrphans(im *longhorn.InstanceManager, instanceMap instanceProcessMap) {
	// TODO: handle engineTargetProcesses
...
func (m *InstanceManagerMonitor) categorizeProcesses(instanceMap instanceProcessMap) (instanceProcessMap, instanceProcessMap, instanceProcessMap) {
	...
	engineTargetProcesses := make(instanceProcessMap)
	...
	for name, process := range instanceMap {
		...
		case longhorn.InstanceTypeEngineTarget:
			engineTargetProcesses[name] = process
		case longhorn.InstanceTypeReplica:
			replicaProcesses[name] = process
		}
	}
	return engineProcesses, engineTargetProcesses, replicaProcesses
}
