...
func (uc *UpgradeController) upgradeInstanceManagers(u *longhorn.Upgrade) (err error) {
	...
	node, err := uc.ds.GetNodeRO(u.Spec.UpgradedNode)
	if err != nil {
	...
	case longhorn.UpgradedNodeStateUndefined:
		log.Infof("Switch upgrade %v to pending state", u.Name)
...
func (uc *UpgradeController) updateVolumesForUpgrade(u *longhorn.Upgrade, log *logrus.Entry) error {
	volumes := map[string]*longhorn.UpgradedVolumeInfo{}
	...
	for _, engine := range engines {
		volumes[engine.Spec.VolumeName] = &longhorn.UpgradedVolumeInfo{
			NodeID: engine.Status.OwnerID,
		}
	}

	replicas, err := uc.ds.ListReplicasByNodeRO(u.Spec.UpgradedNode)
	if err != nil {
		return errors.Wrapf(err, "failed to list volumes for node %v", u.Spec.UpgradedNode)
	}
	for _, replica := range replicas {
		volumes[replica.Spec.VolumeName] = &longhorn.UpgradedVolumeInfo{
			NodeID: replica.Status.OwnerID,
		}
	}

	log.Infof("Switch upgrade %v to upgrading state", u.Name)
	u.Status.Volumes = volumes
...
func (uc *UpgradeController) handleVolumeSuspension(u *longhorn.Upgrade, log *logrus.Entry) error {
	...
	allVolumeRequestedToSuspend := true
	for name := range u.Status.Volumes {
		...
		if volume.Status.OwnerID == u.Spec.UpgradedNode {
		...
		if !reflect.DeepEqual(volume.Spec, existingVolume.Spec) {
			...
			if err != nil {
				...
				allVolumeRequestedToSuspend = false
			}
		}
	}

	if !allVolumeRequestedToSuspend {
		return nil
	}

	allVolumesSuspended := true
	for volumeName := range u.Status.Volumes {
		engine, err := uc.ds.GetVolumeCurrentEngine(volumeName)
		if err != nil {
			return errors.Wrapf(err, "failed to get engine for volume %v", volumeName)
		}
		...
		log.Infof("Volume %v is suspended", volumeName)
	...
	if allVolumesSuspended {
		...
		for name := range u.Status.Volumes {
			...
			if !reflect.DeepEqual(volume.Spec, existingVolume.Spec) {
				_, err := uc.ds.UpdateVolume(volume)
				if err != nil {
					uc.logger.WithError(err).Warnf("Failed to update volume %v for upgrade %v", volume.Name, u.Name)
				}
			}
		}

		log.Infof("Switch upgrade %v to completed state", u.Name)
