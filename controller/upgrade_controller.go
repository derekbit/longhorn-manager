...
func (uc *UpgradeController) handleErr(err error, key interface{}) {
	...
	if uc.queue.NumRequeues(key) < maxRetries {
		handleReconcileErrorLogging(log, err, "Failed to sync Longhorn upgrade resource")
...
func (uc *UpgradeController) reconcile(upgradeName string) (err error) {
	...
	if upgrade.Status.OwnerID != uc.controllerID {
		...
		log.Infof("Upgrade resource %v got new owner %v", upgrade.Name, uc.controllerID)
	}
...
func (uc *UpgradeController) upgradeNode(upgrade *longhorn.Upgrade) (err error) {
	var (
		nodeID        string
		upgradingNode *longhorn.NodeUpgradeStatus
	)
	...
	if node.Spec.UpgradeRequested {
		...
		if condition.Status == longhorn.ConditionStatusTrue {
			log.Infof("UpgradeRequested of node %v is set to true, but it is still schedulable", nodeID)
			return nil
		}
	} else {
	...
	case longhorn.UpgradeStateInitializing:
		...
		volumes, err := uc.collectNodeVolumes(upgrade, nodeID)
		if err != nil {
			// TODO: max retry
			return nil
		}
	...
	case longhorn.UpgradeStateUpgrading:
		logrus.Infof("Debug =======> UpgradeStateUpgrading node %v", nodeID)
...
func (uc *UpgradeController) collectNodeVolumes(upgrade *longhorn.Upgrade, nodeID string) (map[string]*longhorn.VolumeUpgradeStatus, error) {
