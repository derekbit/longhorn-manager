				...
				if r.Spec.Image == v.Status.CurrentImage {
					...
					if types.IsDataEngineV2(r.Spec.DataEngine) {
						if e.Status.CurrentState == longhorn.InstanceStateSuspended {
							if err := c.updateReplicaStateForUpgrade(r); err != nil {
								return err
							}
						}
					}
...
func (c *VolumeController) updateReplicaStateForUpgrade(r *longhorn.Replica) error {
	...
	im, err := c.ds.GetRunningInstanceManagerRO(upgrade.Spec.NodeID, r.Spec.DataEngine)
	if err != nil {
		return errors.Wrapf(err, "failed to get running instance manager for node %v", upgrade.Spec.NodeID)
	}
	...
	if err != nil {
		logrus.Infof("Debug ====> checkReplicaDiskReadiness: diskReady=%v, err=%v", diskReady, err)
	...
	logrus.Infof("Debug ====> im.Spec.Image=%v, defaultInstanceManagerImage=%v", im.Spec.Image, defaultInstanceManagerImage)
...
func (c *VolumeController) upgradeEngineForVolume(v *longhorn.Volume, es map[string]*longhorn.Engine, rs map[string]*longhorn.Replica) error {
	...
	if types.IsDataEngineV1(v.Spec.DataEngine) {
		...
		if err != nil {
			return errors.Wrap(err, "failed to get default instance manager for volume upgrade")
		}
		if im.Status.CurrentState != longhorn.InstanceManagerStateRunning {
			return errors.Wrap(err, "instance manager is not running")
		}

		ready, err := c.allBlockTypeDisksReady(im)
		if err != nil {
			return err
		}
		if !ready {
			log.Warnf("Not all block-type disks managed by instance manager %v are ready", im.Name)
			return nil
		}
...
func (c *VolumeController) allBlockTypeDisksReady(im *longhorn.InstanceManager) (bool, error) {
