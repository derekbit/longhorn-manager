...
func (m *NodeMonitor) getInstanceManager(backendStoreDriver longhorn.BackendStoreDriverType) (*longhorn.InstanceManager, error) {
...
func (m *NodeMonitor) newDiskServiceClient(node *longhorn.Node) (map[longhorn.BackendStoreDriverType]*engineapi.DiskService, error) {
	...
	for _, backendStoreDriver := range backendStoreDrivers {
		im, err := m.getInstanceManager(backendStoreDriver)
		if err != nil {
...
func (m *NodeMonitor) collectDiskData(node *longhorn.Node) map[string]*CollectedDiskInfo {
	...
	diskServiceClients, err := m.newDiskServiceClient(node)
	if err != nil {
