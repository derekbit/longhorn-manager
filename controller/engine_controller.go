...
func (ec *EngineController) DeleteInstance(obj interface{}) (err error) {
	...
	if im.Status.APIVersion == engineapi.IncompatibleInstanceManagerAPIVersion {
		...
		delete(im.Status.InstanceEngines, e.Name)
	...
	err = c.InstanceDelete(e.Name, string(longhorn.InstanceManagerTypeEngine), e.Spec.BackendStoreDriver, "", im.Status.APIVersion)
	if err != nil && !types.ErrorIsNotFound(err) {
		return err
	}
...
func (ec *EngineController) GetInstance(obj interface{}) (*longhorn.InstanceProcess, error) {
	...
	return c.InstanceGet(e.Name, string(longhorn.InstanceManagerTypeEngine), e.Spec.BackendStoreDriver, "", im.Status.APIVersion)
}
...
func (ec *EngineController) LogInstance(ctx context.Context, obj interface{}) (*engineapi.InstanceManagerClient, *imapi.LogStream, error) {
	...
	stream, err := c.InstanceLog(ctx, e.Name, string(longhorn.InstanceManagerTypeEngine), e.Spec.BackendStoreDriver, im.Status.APIVersion)
	return c, stream, err
}
...
func (ec *EngineController) UpgradeEngineProcess(e *longhorn.Engine, log *logrus.Entry) error {
	...
	processBinary, err := c.InstanceGetBinary(e.Name, string(longhorn.InstanceManagerTypeEngine), e.Spec.BackendStoreDriver, "", im.Status.APIVersion)
	if err != nil {
