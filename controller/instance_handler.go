...
func (h *InstanceHandler) syncStatusWithInstanceManager(im *longhorn.InstanceManager, instanceName string, spec *longhorn.InstanceSpec, status *longhorn.InstanceStatus, instances map[string]longhorn.InstanceProcess) {
	...
	case longhorn.InstanceStateRunning:
		logrus.Infof("Debug ===> instance.Status.Type=%v, spec.BackendStoreDriver=%v, spec.DesireState=%v", instance.Status.Type, spec.BackendStoreDriver, spec.DesireState)
		if instance.Status.Type == longhorn.InstanceTypeReplica &&
			spec.BackendStoreDriver == longhorn.BackendStoreDriverTypeSpdkAio &&
			spec.DesireState == longhorn.InstanceStateStopped {
			// For SPDK-AIO replica, a lvol always exists even if the replica is stopped.
			// We can mark the instance as stopped directly.
			if status.Started {
				status.CurrentState = longhorn.InstanceStateError
			} else {
				status.CurrentState = longhorn.InstanceStateStopped
...
func (h *InstanceHandler) ReconcileInstanceState(obj interface{}, spec *longhorn.InstanceSpec, status *longhorn.InstanceStatus) (err error) {
	...
	if im != nil {
		instances, err = h.getInstanceManagerStatusInstanceOf(runtimeObj, im)
		if err != nil {
			return err
		}
	}
...
func (h *InstanceHandler) getInstanceManagerStatusInstanceOf(obj runtime.Object, instanceManager *longhorn.InstanceManager) (map[string]longhorn.InstanceProcess, error) {
	switch obj.(type) {
	case *longhorn.Engine:
		return instanceManager.Status.InstanceEngines, nil
	case *longhorn.Replica:
		return instanceManager.Status.InstanceReplicas, nil
	}
	return nil, fmt.Errorf("unknown type for GetInstanceManagerStatusInstanceOf: %+v", obj)
}
