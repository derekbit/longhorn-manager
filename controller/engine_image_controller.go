...
func (ic *EngineImageController) handleAutoUpgradeEngineImageToDefaultEngineImage(currentProcessingImage string) error {
	...
	for _, vs := range limitedCandidates {
		for _, v := range vs {
			ic.logger.WithFields(logrus.Fields{"volume": v.Name, "engineImage": v.Spec.Image}).Infof("Upgrading volume engine image to the default engine image %v automatically", defaultEngineImage)
