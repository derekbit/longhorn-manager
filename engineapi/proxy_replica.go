...
func (p *Proxy) ReplicaList(e *longhorn.Engine) (replicas map[string]*Replica, err error) {
	resp, err := p.grpcClient.ReplicaList(e.Name, p.DirectToURL(e), string(e.Spec.BackendStoreDriver))
	if err != nil {
		return nil, err
	}
...
func (p *Proxy) ReplicaRebuildStatus(e *longhorn.Engine) (status map[string]*longhorn.RebuildStatus, err error) {
	recv, err := p.grpcClient.ReplicaRebuildingStatus(e.Name, p.DirectToURL(e), string(e.Spec.BackendStoreDriver))
	if err != nil {
		return nil, err
	}
