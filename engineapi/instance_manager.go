...
func (c *InstanceManagerClient) EngineInstanceCreate(e *longhorn.Engine,
	...
	instance, err := c.instanceServiceGrpcClient.InstanceCreate(string(e.Spec.BackendStoreDriver), e.Name, e.Spec.VolumeName,
		string(longhorn.InstanceManagerTypeEngine), "", uint64(e.Spec.VolumeSize),
...
func (c *InstanceManagerClient) ReplicaInstanceCreate(r *longhorn.Replica,
	...
	instance, err := c.instanceServiceGrpcClient.InstanceCreate(string(r.Spec.BackendStoreDriver), r.Name, r.Spec.VolumeName,
		string(longhorn.InstanceManagerTypeReplica), r.Spec.DiskID, uint64(r.Spec.VolumeSize),
...
func (c *InstanceManagerClient) InstanceDelete(backendStoreDriver longhorn.BackendStoreDriverType, name, kind, diskUUID string, cleanupRequired bool) error {
	if c.GetAPIVersion() < 4 {
...
func (c *InstanceManagerClient) InstanceGet(backendStoreDriver longhorn.BackendStoreDriverType, name, kind, diskUUID string) (*longhorn.InstanceProcess, error) {
	if err := CheckInstanceManagerCompatibility(c.apiMinVersion, c.apiVersion); err != nil {
		return nil, err
	}
	...
	instance, err := c.instanceServiceGrpcClient.InstanceGet(string(backendStoreDriver), name, kind, diskUUID)
	if err != nil {
		return nil, err
	}
	return parseInstance(instance), nil
