...
type DataStore struct {
	...
	engineTargetLister             lhlisters.EngineTargetLister
	EngineTargetInformer           cache.SharedInformer
...
func NewDataStore(namespace string, lhClient lhclientset.Interface, kubeClient clientset.Interface, extensionsClient apiextensionsclientset.Interface, informerFactories *util.InformerFactories) *DataStore {
	...
	engineTargetInformer := informerFactories.LhInformerFactory.Longhorn().V1beta2().EngineTargets()
	cacheSyncs = append(cacheSyncs, engineTargetInformer.Informer().HasSynced)
	...
	return &DataStore{
		...
		engineTargetLister:             engineTargetInformer.Lister(),
		EngineTargetInformer:           engineTargetInformer.Informer(),
